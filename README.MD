...
├── requirements.txt
├── vulnerabilities.yml
└── SampleWebAudit/          ← audit folder example passed as argument
    ├── MASS ASSIGMENT.md
    ├── SSTI.md
    ├── MA1.png
    ├── MA2.png
...
The script lives at the root alongside `config.yml`, `requirements.txt`, and `vulnerabilities.yml` (your examples file). Each audit is a subfolder containing all `.md` notes and screenshots together — no subdirectory nesting needed.
...
| Model | Speed | Quality |
|---|---|---|
| `claude-haiku-4-5` | ⚡ fastest | good — **default** |
| `claude-sonnet-4-5` | fast | better |
...
Up to 5 examples are included in the prompt.
...
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml

# English output, Gemini provider
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml --provider gemini --lang en

# Use a more powerful Claude model
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml --model claude-opus-4-5

# Inject extra instructions into the prompt
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml --instructions "Always include the CWE identifier in the description."

# Skip interactive audit selection
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml --audit-id 6657f1a2c3b4e500123abc

# Test without uploading anything
python ai-pwndoc.py SampleWebAudit/ -e vulnerabilities.yml --dry-run
